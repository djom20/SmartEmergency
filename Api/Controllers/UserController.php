...
class UserController extends ControllerBase {
    ...
    function add() {
        ...
        if ($filled && $empty) {
            ...
            if ($usuario->lastID() > 0) {
                ...
                $sw = mail($this->post['correo'], 'Bienvenido a Pilotos', $mail, $headers);
    ...
    function send(){
        ...
        if(count($result) == 1){
            ...
            mail($user_list, 'Creado Proceso '.$this->post['idproceso'].' - Pilotos', $html, $headers);
            HTTP::JSON(200);
        }
    ...
    function remainder($idproceso){
        ...
        mail($user_list, 'Proceso '.$idproceso.' - Pilotos', $html, $headers);
        HTTP::JSON(200);
    }
    ...
    function restore () {
        if(!empty ($this->post['correo'])) {
            ...
            if(count($result) == 1) {
                ...
                mail($this->post['correo'], 'Restaurar clave - Pilotos', $mail, $headers);
    ...
    function changeState() {
        if($_SESSION['usuario']['idusuario'] != $this->post['idusuario']){
            ...
            HTTP::JSON(Partial::createResponse(HTTP::Value(200), $_SESSION['usuario']. ' '. $this->post['idusuario']));
        }
        HTTP::JSON(400);
    }
